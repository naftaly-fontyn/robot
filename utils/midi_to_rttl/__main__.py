...
# ---------------------------------------------------------------------
MIDI_FILENAME = "IRIS OUT.mid" # <--- EDIT THIS LINE (e.g., "MySong.mid")
...
def convert_midi_to_arrays(filename):
    """ Convert to python data, freq, duration tempo, dur,note,oct """
    ...
    for msg in track:
        ...
        if msg.type == 'set_tempo':
            tempo_us = msg.tempo
            print("tempo_us:", tempo_us, mido.tempo2bpm(tempo_us))  # Todo: here is bps
        ...
        if is_note_on:
            ...
            if note in held_notes:
                ...
                if duration_ticks > 0:
                    ...
                    duration = round(ticks_to_ms(duration_ticks, tempo_us, tpb))
                    b = round(duration / tempo_us * 1000 * 8)
                    s = ',' if l%8 else '|'
                    l += b // 2
                    print(f'{s}{b}{macro_name[5:]}', end='')
                    melody_macros.append(macro_name)
                    duration_ms.append(duration)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print('Usage: python app.py <MIDI_FILENAME>')
