...
class ClientUi(tk.Frame):
    ...
    def do_us_scan_btn(self):
        ...
        print(response)
        if response[2] is None or 'message' not in response[2]:
            print(f"US Scan failed: {response}")
            return
...
def start_embedded_broker():
    """
    Starts the AMQTT broker in a background thread.
    Silences all warnings and logs for a clean console.
    """
    def broker_worker():
        ...
        # 2. Configuration (Bind to Localhost for reliability)
        config = {
            'listeners': {
                'default': {
                    'type': 'tcp',
            ...
            'auth': {
                ...
                'password-file': '', # Silence 'parameter not found' log
            }
        }

        async def run():
            # 3. Silence Noise
            # Filter the specific DeprecationWarning from amqtt
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            # Mute amqtt's internal logger (hides "password-file" warning)
        ...
        try:
            ...
            print(f"[Broker] Crash: {e}")
        finally:
            loop.close()

    t = threading.Thread(target=broker_worker, daemon=True)
    t.start()


if __name__ == "__main__":
    print("--- 1. Connecting Client ---")
    init_client()

    print("--- 2. Launching GUI ---")
    root = tk.Tk()
