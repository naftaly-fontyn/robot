...
import utime
import uos
from boards.matrixbit_on3.nano_gui.gui.core.writer import Writer, CWriter
from boards.matrixbit_on3.nano_gui.gui.core.fplot import PolarGraph, PolarCurve, CartesianGraph, Curve, TSequence
from boards.matrixbit_on3.nano_gui.gui.core.nanogui import refresh
from boards.matrixbit_on3.nano_gui.gui.widgets.label import Label

refresh(ssd, True)

# Fonts
import boards.matrixbit_on3.nano_gui.gui.fonts.arial10 as arial10
import boards.matrixbit_on3.nano_gui.gui.fonts.freesans20 as freesans20

from boards.matrixbit_on3.nano_gui.gui.core.colors import *
...
def cart():
    ...
    g = CartesianGraph(wri, 2, 2, yorigin = 2, fgcolor=WHITE, gridcolor=LIGHTGREEN,height=60,) # Asymmetric y axis
...
def polar():
    ...
    refresh(ssd, True)  # Clear any prior image
    g = PolarGraph(wri, 2, 2, fgcolor=WHITE, gridcolor=LIGHTGREEN,height=60,)
...
def rt_rect():
    ...
    refresh(ssd, True)  # Clear any prior image
    g = CartesianGraph(wri, 2, 2, fgcolor=WHITE, gridcolor=LIGHTGREEN,height=60,)
    curve = Curve(g, RED)
    x = -1
    for _ in range(40):
...
def lem():
    ...
    g = CartesianGraph(wri, 2, 2, height = 60, fgcolor=WHITE, gridcolor=LIGHTGREEN)
...
def liss():
    ...
    def populate():
        t = -math.pi
        while t <= math.pi:
            yield math.sin(t) * scl, math.cos(3*t) * scl # x, y
            t += 0.1
    refresh(ssd, True)  # Clear any prior image
    g = CartesianGraph(wri, 2, 2, fgcolor=WHITE, gridcolor=LIGHTGREEN,height=60,)
    curve = Curve(g, YELLOW, populate())
...
def seq():
    ...
    for t in range(100):
        g.clear()
        tsy.add(0.9*math.sin(t/10) * scl)
        tsr.add(0.4*math.cos(t/10)* scl)
        refresh(ssd)
        utime.sleep_ms(100)
